...
from legal_rag.common.paths import OUTPUTS_DIR
from legal_rag.common.qdrant import create_qdrant_client

_LOG_LOCK = threading.Lock()
_PROGRESS_LOG = OUTPUTS_DIR / "embed_asof_20260301_v1_progress.jsonl"
_ERROR_LOG = OUTPUTS_DIR / "embed_asof_20260301_v1_errors.jsonl"
_MISSING_IDS_LOG = OUTPUTS_DIR / "embed_asof_20260301_v1_missing_ids.json"


def _acquire_collection_lock(collection: str) -> Path:
    safe_name = "".join(ch if ch.isalnum() or ch in "-_" else "_" for ch in collection)
    lock_path = OUTPUTS_DIR / f".{safe_name}.embedding.running"
